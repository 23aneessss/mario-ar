...
export default function CameraApp() {
  ...
  return <main className="camera-app">
    ...
    <Dialog open={help} onClose={()=>setHelp(false)} title="Aide"><div className="help-content"><ol><li><strong>Choisissez un élément</strong><p>Ouvrez le catalogue et touchez un modèle disponible.</p></li><li><strong>Composez votre photo</strong><p>Glissez pour déplacer. Pincez pour redimensionner. Tournez deux doigts pour incliner. La barre de réglage permet aussi de changer sa direction.</p></li><li><strong>Capturez et emportez</strong><p>Téléchargez immédiatement. Si le partage est disponible, scannez le QR code depuis un autre appareil.</p></li></ol><p className="notice-text">Les objets restent dans le cadre de l’écran lorsque vous bougez. Ils ne sont pas ancrés dans la pièce.</p><h3>Crédits des modèles</h3>{catalog.filter(a=>a.status==='available').map(a=><p key={a.id}><a href={a.source!} target="_blank" rel="noreferrer">{a.name}</a> · {a.author} · <a href={a.licenseUrl!} target="_blank" rel="noreferrer">{a.license}</a><br/><small>{a.note}</small></p>)}<p>Les objets « Démo » sont procéduraux. Les personnages manquants seront activés à l’ajout de leurs fichiers.</p><p className="privacy-note">{config.reason} Les photos partagées expirent après {config.days} jours. Aucune galerie publique.</p></div></Dialog>
